    ...
    try:
        r = subprocess.run(
            ["nvidia-smi", "--query-gpu=index,name,uuid,pci.bus_id,compute_cap,memory.total,memory.free,driver_version",
             "--format=csv,noheader,nounits"], capture_output=True, text=True, errors='replace', timeout=10)
...
def _get_cuda_version() -> str:
    ...
    try:
        r = subprocess.run(["nvidia-smi"], capture_output=True, text=True, errors='replace', timeout=5)
        if r.returncode == 0:
