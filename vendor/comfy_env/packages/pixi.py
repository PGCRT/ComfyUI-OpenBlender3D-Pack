...
def _sanitize_dist_info_metadata(env_root: Path, log: Callable[[str], None] = print) -> int:
    """Normalize problematic local-version metadata that breaks pixi parser."""
    candidates = [
        env_root / "Lib" / "site-packages",
        env_root / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages",
        env_root / ".pixi" / "envs" / "default" / "Lib" / "site-packages",
        env_root / ".pixi" / "envs" / "default" / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages",
    ]
    fixed = 0
    for site_packages in candidates:
        ...
        for dist_info in site_packages.glob("*.dist-info"):
            ...
            if "+" not in version or "torch" not in version:
                continue
            ...
            if new_text != text:
                try:
                    ...
                    pass
    if fixed:
        log(f"[comfy-env] Sanitized {fixed} dist-info METADATA version(s)")
    return fixed


def _sanitize_for_node_dir(node_dir: Path, log: Callable[[str], None] = print) -> None:
    _sanitize_dist_info_metadata(node_dir, log)
    try:
        from ..environment.cache import resolve_env_path

        env_path, _, _ = resolve_env_path(node_dir)
        if env_path:
            _sanitize_dist_info_metadata(env_path, log)
    except Exception:
        pass
...
def pixi_install(node_dir: Path, log: Callable[[str], None] = print) -> subprocess.CompletedProcess:
    ...
    _sanitize_for_node_dir(node_dir, log)
