...
_VERBOSE = _DEBUG or os.environ.get("OPENBLENDER3D_VERBOSE", "").lower() in ("1", "true", "yes", "debug")
...
_NOISY_LOG_TOKENS = (
    ...
    "DinoV3 conditioning:",
    "DinoV3 load:",
    "DinoV3 512px extract:",
    "DinoV3 1024px extract:",
    "soft_empty_cache:",
    "Built SparseStructure",
    "Built SLatFlowModel",
    "Built Flexi",
    "Built SparseUnet",
    "-> GPU:",
...
def _is_noisy_worker_log(message: str) -> bool:
    if _VERBOSE:
        return False
    return any(token in message for token in _NOISY_LOG_TOKENS)

def _rewrite_worker_log_message(message: str) -> str:
    if "You need pytorch with cu130 or higher to use optimized CUDA operations" in message:
        return "Optimized CUDA operations fallback: using compatible PyTorch/CUDA path."
    return message
...
def _to_shm(obj, registry, visited=None):
    ...
    if t == 'Tensor':
        ...
        if obj.is_cuda and _probe_cuda_ipc():
            ...
            # Don't cache tensor serialization in visited to avoid id-recycling bugs
            # when temporary tensors are garbage collected and reused.
            return result
    ...
    # MeshWithVoxel -> explicit attribute serialization (survives worker restarts)
    if t == 'MeshWithVoxel':
        import torch
        result = {
            "__mesh_with_voxel__": True,
            "vertices": _to_shm(obj.vertices.detach().cpu().contiguous(), registry, visited),
            "faces": _to_shm(obj.faces.detach().cpu().contiguous(), registry, visited),
            "origin": _to_shm(obj.origin.detach().cpu().contiguous() if isinstance(obj.origin, torch.Tensor) else torch.tensor(obj.origin, dtype=torch.float32), registry, visited),
            "voxel_size": obj.voxel_size,
            "coords": _to_shm(obj.coords.detach().cpu().contiguous(), registry, visited),
            "attrs": _to_shm(obj.attrs.detach().cpu().contiguous(), registry, visited),
            "voxel_shape": list(obj.voxel_shape),
            "layout": _to_shm(obj.layout, registry, visited),
        }
        visited[obj_id] = result
        return result
    ...
    # slice -> serializable representation
    if isinstance(obj, slice):
        return {"__shm_slice__": True, "start": obj.start, "stop": obj.stop, "step": obj.step}
...
def _from_shm(obj, unlink=True):
    ...
    # MeshWithVoxel -> reconstruct from serialized attributes
    if "__mesh_with_voxel__" in obj:
        import torch
        vertices = _from_shm(obj["vertices"], unlink)
        faces = _from_shm(obj["faces"], unlink)
        origin = _from_shm(obj["origin"], unlink)
        coords = _from_shm(obj["coords"], unlink)
        attrs = _from_shm(obj["attrs"], unlink)
        layout = _from_shm(obj.get("layout", {}), unlink)
        try:
            from nodes.trellis2.representations.mesh.base import MeshWithVoxel
            return MeshWithVoxel(
                vertices=vertices,
                faces=faces,
                origin=origin.tolist() if isinstance(origin, torch.Tensor) else origin,
                voxel_size=obj["voxel_size"],
                coords=coords,
                attrs=attrs,
                voxel_shape=torch.Size(obj["voxel_shape"]),
                layout=layout,
            )
        except Exception:
            try:
    ...
    if "__shm_sparse_tensor__" in obj:
        ...
        coords = _from_shm(obj["coords"], unlink)
        try:
            from nodes.trellis2.modules.sparse import SparseTensor
...
# Add torch lib directory to PATH for compiled extensions (pytorch3d, etc.)
# This must happen BEFORE importing torch or any compiled extension
if sys.platform == "win32":
    _torch_lib = os.path.join(os.path.dirname(sys.executable), "Lib", "site-packages", "torch", "lib")
    if os.path.isdir(_torch_lib):
        # Append (not prepend) torch/lib to PATH to avoid shadowing the system
        # / pixi VC++ runtime that OpenCV needs. DLLs already loaded by torch
        # are found via module handle, so append is safe for torch extensions.
        os.environ["PATH"] = os.environ.get("PATH", "") + ";" + _torch_lib
...
def _rewrite_worker_log_message(message):
    if "You need pytorch with cu130 or higher to use optimized CUDA operations" in message:
        return "Optimized CUDA operations fallback: using compatible PyTorch/CUDA path."
    return message
...
# Pre-load cv2 before torch to avoid VC++ runtime DLL conflicts on Windows.
# PyTorch's bundled runtime DLLs in torch/lib can shadow the system/pixi
# runtime that OpenCV needs, causing "procedure not found" errors.
if sys.platform == "win32":
    try:
        import cv2
        wlog(f"[worker] cv2 pre-loaded: {cv2.__version__}")
    except Exception as _cv2_err:
        wlog(f"[worker] cv2 pre-load skipped: {_cv2_err}")
...
try:
    ...
    # Add torch lib directory to DLL search path for compiled extensions (pytorch3d, etc.)
    _torch_lib = os.path.join(os.path.dirname(torch.__file__), "lib")
    if os.path.isdir(_torch_lib) and hasattr(os, "add_dll_directory"):
        try:
            os.add_dll_directory(_torch_lib)
            wlog(f"[worker] Added torch lib to DLL search: {_torch_lib}")
        except Exception as _e:
            wlog(f"[worker] Failed to add torch lib: {_e}")
except Exception as e:
...
def _to_shm(obj, registry, visited=None):
    ...
    if t == 'SparseTensor':
        ...
        result = {
            "__shm_sparse_tensor__": True,
            "coords": _to_shm(_c, registry, visited),
            "feats": _to_shm(_f, registry, visited),
            "feats_dtype": str(_f.dtype),
        }
        visited[obj_id] = result
        return result
    ...
    # Keep custom pipeline/model/helper objects inside the persistent worker.
    # The parent receives a JSON-safe reference token and passes it back to
    # downstream isolated nodes, where _deserialize_input resolves it.
    if _should_use_reference(obj):
        ref_id = _cache_object(obj)
        result = {"__comfy_ref__": ref_id, "__class__": type(obj).__name__}
        visited[obj_id] = result
        return result
...
def _from_shm(obj, _depth=0, _key="root"):
    ...
    # MeshWithVoxel -> reconstruct from serialized attributes
    if "__mesh_with_voxel__" in obj:
        wlog(f"[_from_shm] {_key}: MeshWithVoxel")
        import torch
        vertices = _from_shm(obj["vertices"], _depth+1, f"{_key}.vertices")
        faces = _from_shm(obj["faces"], _depth+1, f"{_key}.faces")
        origin = _from_shm(obj["origin"], _depth+1, f"{_key}.origin")
        coords = _from_shm(obj["coords"], _depth+1, f"{_key}.coords")
        attrs = _from_shm(obj["attrs"], _depth+1, f"{_key}.attrs")
        layout = _from_shm(obj.get("layout", {}), _depth+1, f"{_key}.layout")
        try:
            from nodes.trellis2.representations.mesh.base import MeshWithVoxel
            return MeshWithVoxel(
                vertices=vertices,
...
def _should_use_reference(obj):
    ...
    if obj_type == 'Trimesh':
        return False
    # MeshWithVoxel - pass by value via pickle (survives worker restarts)
    if obj_type == 'MeshWithVoxel':
        return False
    # slice objects - pass by value (they pickle well and are common in layout dicts)
    if obj_type == 'slice':
        return False
...
def main():
    ...
    try:
        ...
        # Add torch lib directory to DLL search path for compiled extensions
        import os as _os
        ...
        if _os.path.isdir(torch_lib):
            ...
            wlog(f"[worker] Added torch lib to DLL path: {torch_lib}")
    except Exception as e:
        wlog(f"[worker] Torch not available: {e}")

    # Force comfy_sparse_attn to use SDPA fallback (sage2/flash can corrupt shapes on torch 2.8+cu128)
    try:
        import comfy_sparse_attn.varlen as _csa_varlen
        _csa_varlen._varlen_fn = _csa_varlen._sdpa_varlen
        _csa_varlen._varlen_backend = "sdpa"
        wlog("[worker] Forced comfy_sparse_attn varlen backend to sdpa")
    except Exception:
        pass
    ...
    def _forwarded_print(*args, **kwargs):
        ...
        message = _rewrite_worker_log_message(message)
        if _is_noisy_worker_log(message):
            wlog(f"[print-filtered] {message}")
            return
        # Send to host
        try:
    ...
    class SocketLogHandler(logging.Handler):
        def emit(self, record):
            try:
                ...
                msg = _rewrite_worker_log_message(msg)
                if _is_noisy_worker_log(msg):
                    wlog(f"[log-filtered] {msg}")
                    return
    ...
    if not any(getattr(h, "_comfy_env_socket_handler", False) for h in logging.root.handlers):
        _socket_handler._comfy_env_socket_handler = True
    ...
    # ---------------------------------------------------------------
    # Attention backend auto-enable is DISABLED to prevent segfaults
    # with compiled attention kernels (sage/flash) on certain torch/CUDA
    # combinations (e.g. PyTorch 2.8 + CUDA 12.8). Workers fall back to
    # PyTorch native SDPA which is stable across all configurations.
    #
    # Users can still opt-in by passing --use-sage-attention or
    # --use-flash-attention to the main ComfyUI process.
    # ---------------------------------------------------------------
    pass
    ...
    while True:
        ...
        try:
            ...
            # Clear cached top-level packages to prevent import collisions
            # when multiple components share the same worker but have different package roots.
            # (e.g. motioncapture and sam3dbody both use 'nodes' as their package name)
            top_pkg = module_name.split('.')[0]
            to_remove = [k for k in sys.modules if k == top_pkg or k.startswith(top_pkg + '.')]
            for k in to_remove:
                del sys.modules[k]
...
class SubprocessWorker(Worker):
    ...
    def _ensure_started(self):
        ...
        # Launch subprocess with the resolved env Python, passing socket address.
        # Older builds used "pixi run python" for pixi envs, but that can resolve
        # to an unrelated user Python when the manifest environment is already
        # materialized elsewhere. The resolved env python is deterministic and
        # build_isolation_env() supplies the DLL/PATH activation bits we need.
        use_pixi_run = os.environ.get("COMFY_ENV_USE_PIXI_RUN", "").lower() in ("1", "true", "yes")
        is_pixi = use_pixi_run and '.pixi' in str(self.python)
        if _DEBUG:
        ...
        if is_pixi:
            ...
            if pixi_toml.exists():
                ...
                # Use -s (no user site) to prevent user site-packages (e.g.
                # AppData\Roaming\Python\Python311\site-packages) from shadowing
                # packages in the isolated env. This fixes DLL mismatches where
                # pytorch3d._C was compiled against the env's torch but loads a
                # different torch version from user site.
                cmd = [str(pixi_exe), "run", "--manifest-path", str(pixi_toml),
                       "python", "-s", str(self._worker_script), self._socket_addr]
                ...
                # Also block user site via env var as a safety net
                env["PYTHONNOUSERSITE"] = "1"
                launch_env = env
            else:
                cmd = [str(self.python), "-s", str(self._worker_script), self._socket_addr]
                env["PYTHONNOUSERSITE"] = "1"
                launch_env = env
        else:
            cmd = [str(self.python), "-s", str(self._worker_script), self._socket_addr]
        ...
        try:
            while True:
                ...
                if msg.get("type") == "log":
                    ...
                    log_msg = _rewrite_worker_log_message(log_msg)
                    if not _is_noisy_worker_log(log_msg):
                        print(f"[worker:{self.name}] {log_msg}", file=sys.stderr, flush=True)
                    continue
                break
        ...
        try:
            while True:
                ...
                if response.get("type") == "log":
                    ...
                    msg = _rewrite_worker_log_message(msg)
                    if not _is_noisy_worker_log(msg):
