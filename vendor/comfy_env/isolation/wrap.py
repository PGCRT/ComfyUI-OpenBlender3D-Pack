...
_WORKER_PATCHERS: Dict[
    str, Dict[str, Any]
] = {}  # str(env_dir) -> {model_id: SubprocessModelPatcher}
            ...
            try:
                ...
                if any("persistent_worker.py" in arg for arg in cmdline):
                    ...
                    if parent_pid and not psutil.pid_exists(parent_pid):
                        print(
                            f"[comfy-env] Killing orphaned worker (parent {parent_pid} dead): {proc.pid}"
                        )
                        proc.kill()
                ...
                try:
                    ...
                    if "persistent_worker.py" in cmdline:
                        ...
                        if not os.path.exists(f"/proc/{ppid}"):
                            print(
                                f"[comfy-env] Killing orphaned worker (parent {ppid} dead): {pid}"
                            )
...
def _build_isolation_env_win32(env: dict, python: Path) -> dict:
    ...
    # Add torch lib directory for compiled extension DLLs (pytorch3d, etc.)
    torch_lib = env_root / "Lib" / "site-packages" / "torch" / "lib"
    if torch_lib.is_dir():
        minimal_path_parts.insert(1, str(torch_lib))
...
def _build_isolation_env_linux(env: dict, python: Path) -> dict:
    ...
    if lib_dir.is_dir():
        ...
        env["LD_LIBRARY_PATH"] = (
            f"{lib_dir}:{system_libs}:{existing}" if existing else f"{lib_dir}:{system_libs}"
        )
    return env
...
def _should_share_torch(env_dir: Path) -> bool:
    ...
    DISABLED: This feature was causing CUDA torch issues.
    Always return False to ensure torch is installed in the isolated environment
    with proper CUDA support instead of trying to share from host.
    """
...
def _handle_vram_budget(request: dict) -> dict:
    ...
    if _DEBUG:
        ...
        _log(
            f"[comfy-env] VRAM request: {total_requested / 1e9:.2f}GB, "
...
def _worker_key(env_dir, working_dir):
    """Unique key for worker pool: env + working dir prevents cross-component collisions."""
    return f"{env_dir}:{working_dir}"


def _cleanup_stale_patchers(key):
    ...
    if not old_patchers:
        return
    _log(
        f"[comfy-env] Invalidated {len(old_patchers)} stale model patchers "
...
def _get_or_create_worker(
    ...
    key = _worker_key(env_dir, working_dir)
    with _POOL_LOCK:
        ...
        worker = _create_worker(
            env_dir, working_dir, sys_path, lib_path, env_vars, health_check_timeout
        )
        ...
        worker._on_restart = lambda _key=key: _cleanup_stale_patchers(_key)
        _WORKER_POOL[key] = (worker, gen)
        return worker, gen


def _remove_worker(env_dir, working_dir):
...
def _register_new_patchers(env_dir, working_dir, worker, generation):
...
def _load_worker_models(env_dir, working_dir):
...
def _clear_worker_patchers(env_dir, working_dir):
    """Forget model patchers for a live worker without killing the worker.

    Used by nodes that explicitly offload models inside worker execution
    (e.g. Pixal3D force_offload mode), to prevent reloading models on the next
    call while preserving in-worker object references for downstream nodes.
    """
    key = _worker_key(env_dir, working_dir)
    with _POOL_LOCK:
        _WORKER_PATCHERS.pop(key, None)
...
def register_nodes(nodes_package: str = "nodes") -> tuple:
    ...
    configured_dirs = {cf.parent.resolve() for cf in config_files if cf.name != "comfy-env-root.toml"}
    ...
    elif root_resolved not in isolation_envs and root_resolved not in configured_dirs:
    ...
    if not all_mappings and root_resolved not in isolation_envs and root_resolved not in configured_dirs:
        main_dirs = []
        isolation_dirs = []
        ...
        if enabled and isolation_dirs:
            ...
            with ThreadPoolExecutor(max_workers=len(isolation_dirs)) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if nodes_meta:
                        _log(
                            f"[comfy-env] Registered {len(nodes_meta)} isolation nodes from {subdir.name}"
                        )
