...
try:
    ...
    Log.warning(
        f"[GVHMRInference] DPVO import failed ({type(exc).__name__}: {exc}) - only SimpleVO will be available"
    )
...
class GVHMRInference(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                io.Image.Input("images",
                    tooltip="Batch of input images (B, H, W, 3) as torch tensors from ComfyUI image nodes."),
                io.Mask.Input("masks",
                    tooltip="Batch of mask images (B, H, W) as torch tensors where the person is white on black background (e.g. from SAM3 or segmentation nodes). Must match image batch size."),
    ...
    def _load_models(config: Dict) -> Dict:
        ...
        # Apply attention backend override if requested (prevents sage/flash segfaults on some torch/cuda combos)
        attention = config.get("attention", "auto")
        if attention != "auto":
            try:
                ...
                if attention == "sdpa":
                    ...
                    Log.info("[GVHMRInference] Attention backend forced to: sdpa (pytorch)")
                elif attention == "flash_attn" and hasattr(_attn, "attention_flash"):
                    _attn.optimized_attention = _attn.attention_flash
                    _attn.optimized_attention_masked = _attn.attention_flash
                    Log.info("[GVHMRInference] Attention backend forced to: flash_attn")
                elif attention == "sage" and hasattr(_attn, "attention_sage"):
                    _attn.optimized_attention = _attn.attention_sage
                    _attn.optimized_attention_masked = _attn.attention_sage
                    Log.info("[GVHMRInference] Attention backend forced to: sage")
                else:
                    Log.warning(f"[GVHMRInference] Unknown or unavailable attention backend: {attention}, using auto")
            except Exception as e:
                Log.warning(f"[GVHMRInference] Failed to set attention backend: {e}")
        else:
            # Auto-detect known-broken torch/cuda combinations and fall back to sdpa to avoid segfaults
            try:
                import torch as _torch_ver
                if _torch_ver.__version__.startswith("2.8") and hasattr(_torch_ver, 'version') and _torch_ver.version.cuda == "12.8":
                    import comfy.ldm.modules.attention as _attn
                    _attn.optimized_attention = _attn.attention_pytorch
                    _attn.optimized_attention_masked = _attn.attention_pytorch
                    Log.info("[GVHMRInference] Auto-detected torch 2.8+cu128 with known sage/flash instability, forcing sdpa")
            except Exception:
                pass
    ...
    def prepare_data_from_images(
        ...
        """
        Prepare data dictionary for GVHMR inference from IMAGE and MASK tensor batches.
        
        Args:
            images: ComfyUI IMAGE tensor (B, H, W, 3) float [0, 1]
            masks: ComfyUI MASK tensor (B, H, W) float [0, 1]
        """
        import comfy.model_management
        ...
        # IMAGE: (B, H, W, 3) float [0, 1] -> (B, H, W, 3) uint8 RGB
        if isinstance(images, torch.Tensor):
            images_np = (images.cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
        else:
            images_np = images
        
        # MASK: (B, H, W) float [0, 1] -> (B, H, W) uint8 grayscale
        if isinstance(masks, torch.Tensor):
            masks_np = (masks.cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
        ...
        mask_height, mask_width = masks_np.shape[1:3]

        Log.info(f"[GVHMRInference] Images: {batch_size} frames @ {width}x{height}")
        Log.info(f"[GVHMRInference] Masks: {batch_size} frames @ {mask_width}x{mask_height}")
        Log.info(f"[GVHMRInference] Processing {batch_size} frames, static_camera={static_camera}")
        _log_memory("Start of prepare_data_from_images")

        # --- Phase 1: Extract bounding boxes from mask frames ---
        Log.info("[GVHMRInference] Phase 1: Extracting bounding boxes from mask frames...")
        bboxes_xywh = []
        for frame_idx in range(batch_size):
            comfy.model_management.throw_exception_if_processing_interrupted()
            mask_frame = masks_np[frame_idx]  # (H, W) uint8
            bbox = extract_bbox_from_numpy_mask(mask_frame)
            bboxes_xywh.append(bbox)

        Log.info(f"[GVHMRInference] Extracted {len(bboxes_xywh)} bounding boxes")
        _log_memory("After bbox extraction from mask frames")

        # Scale bboxes if mask and video have different resolutions
        if (mask_width, mask_height) != (width, height):
            Log.warn(f"[GVHMRInference] Mask resolution ({mask_width}x{mask_height}) != image resolution ({width}x{height}). Scaling bboxes.")
            scale_x = width / mask_width
            scale_y = height / mask_height
            bboxes_xywh = [
                [int(b[0] * scale_x), int(b[1] * scale_y), int(b[2] * scale_x), int(b[3] * scale_y)]
        ...
        # --- Pass 1: ViTPose (process chunks + crop + extract keypoints) ---
        comfy.model_management.load_models_gpu([model_bundle["vitpose_patcher"]])
        _log_memory("ViTPose on GPU")
        ...
        for frame_idx in range(batch_size):
            comfy.model_management.throw_exception_if_processing_interrupted()
            frame_np = images_np[frame_idx]  # (H, W, 3) uint8 RGB
            chunk_frames.append(frame_np)

            # Process chunk when full or at end
            if len(chunk_frames) >= CHUNK_SIZE or frame_idx >= batch_size - 1:
                chunk_end = chunk_start + len(chunk_frames)
                Log.info(f"[GVHMRInference] ViTPose chunk [{chunk_start}:{chunk_end}] ({len(chunk_frames)} frames)")
        ...
        _log_memory("End of prepare_data_from_images")
        return data, camera_data
    ...
    def execute(
        ...
        """
        Run GVHMR inference on image batch with mask batch.
        """
        import comfy.model_management
        try:
            static_camera = not moving_camera
            
            # Get batch info from tensors
            img_batch = images.shape[0] if hasattr(images, 'shape') else len(images)
            mask_batch = masks.shape[0] if hasattr(masks, 'shape') else len(masks)
            
            Log.info("=" * 60)
            Log.info("[GVHMRInference] Starting GVHMR inference with parameters:")
            Log.info(f"  images:          {img_batch} frames")
            Log.info(f"  masks:           {mask_batch} frames")
            ...
            # Prepare data (processes image/mask tensor batches)
            data, camera_data = cls.prepare_data_from_images(
                images, masks, model, static_camera, focal_length_mm, bbox_scale, vo_method, vo_scale, vo_step, intrinsics, dpvo_dir, chunk_size
            )
